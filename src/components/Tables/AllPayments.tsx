  ...
  const currentlyAdmitted = async () => {
    try {
      ...
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_API_URL}/all_payments`,
        {
          headers: {
      ...
      setIncompleteApplications(response.data.allPayments);
      setFilteredIncompleteApplications(response.data.allPayments); // Set initial filtered courses
      setLoading(false);
    } catch (err) {
