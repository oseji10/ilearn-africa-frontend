  ...
  const currentlyAdmitted = async () => {
    try {
      ...
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_API_URL}/currently_admitted`,
        {
          headers: {
      ...
      setIncompleteApplications(response.data.currentlyAdmitted);
      setFilteredIncompleteApplications(response.data.currentlyAdmitted); // Set initial filtered courses
      setLoading(false);
    } catch (err) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          ...
          >
            ...
            <div className="mb-4">
              ...
              {selectedIncompleteApplications.payments?.courses?.course_id} - {selectedIncompleteApplications.payments?.courses?.course_name} 
            </div>
...
export default CurrentlyAdmitted;
