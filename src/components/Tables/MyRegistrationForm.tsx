...
const [success, setSuccess] = useState("");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      router.push('/client-dashboard/my-registration/registration-complete')
    } catch (error) {
      console.error(
        "Error:",
...
const uploadFile = async (file) => {
  ...
  try {
    await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/upload-document`, formData, {
      headers: {
        Authorization: `Bearer ${token}`,
            ...
            <div className="mt-5">
              {file && file.type.startsWith('image/') ? (
                ...
                >
                  ...
                  <button
                    className="mt-5.5  rounded bg-primary p-3 font-medium text-gray"
                    type="submit"
                  >
                    Submit
                  </button>
