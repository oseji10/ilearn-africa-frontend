  ...
  const currentlyAdmitted = async () => {
    try {
      ...
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_API_URL}/payments_this_week`,
        {
          headers: {
      ...
      setIncompleteApplications(response.data.paymentsThisWeek);
      setFilteredIncompleteApplications(response.data.paymentsThisWeek); // Set initial filtered courses
      setLoading(false);
    } catch (err) {
