  ...
  const currentlyAdmitted = async () => {
    try {
      ...
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_API_URL}/payments_today`,
        {
          headers: {
      ...
      setIncompleteApplications(response.data.paymentsToday);
      setFilteredIncompleteApplications(response.data.paymentsToday); // Set initial filtered courses
      setLoading(false);
    } catch (err) {
